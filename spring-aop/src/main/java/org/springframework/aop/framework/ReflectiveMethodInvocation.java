...
public class ReflectiveMethodInvocation implements ProxyMethodInvocation, Cloneable {
	 ...
	 * that need dynamic checks. <><p/>
	 * 拦截器：
	 * 1、MethodInterceptor 或
	 ...
	 * -1 until we invoke: then the current interceptor.<p/>
	 * 执行索引，用来顺序调用拦截器链使用
	 */
	...
	public Object proceed() throws Throwable {
		...
		if (interceptorOrInterceptionAdvice instanceof InterceptorAndDynamicMethodMatcher) { // 动态匹配的增强器
			...
			if (dm.methodMatcher.matches(this.method, targetClass, this.arguments)) { // 匹配，执行拦截器
				...
				// 不匹配就不执行拦截器，触发下一个拦截器的判断和执行
				return proceed();
			}
		}
		else { // 非动态拦截器，比如事务的TransactionInterceptor，和异步的AnnotationAsyncExecutionInterceptor等等
